...
from tools.api_client import fetch_stats
...
class StatsCog(commands.Cog, name=CogsNames.STATS):
    ...
    async def game_ccu(self, interaction: discord.Interaction):
        ...
        updated_at, regions, global_stats = await fetch_stats()
        if not regions:
        ...
        for region in regions:
            ...
            lines.append(
                f"🔹 **`{region.name}`** ({DefaultEmojis.ONLINE if region.status == "ok" else f"Status: {region.status}"}) ({region.total} players in total)\n"
        ...
        stats_text = "\n".join(lines).strip()
        container.add_item(discord.ui.TextDisplay(content=f"Total players around the world: **{global_stats.total}**"))
